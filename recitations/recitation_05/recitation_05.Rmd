...
anova <- aov(netContentsPaymentAmount ~ occupancyType, data = claims_housing)
...
plot(katrina_claims_filt$netBuildingPaymentAmount, katrina_claims_filt$buildingDamageAmount)
```
...
```{r}
ggplot(katrina_claims_filt, aes(x=netBuildingPaymentAmount, y=buildingDamageAmount)) +
...
```{r}
katrina_claims <- lm(netBuildingPaymentAmount ~ buildingDamageAmount, data = katrina_claims)

summary(katrina_claims)
```
